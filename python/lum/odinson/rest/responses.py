...
class CorpusInfo(BaseModel):
    ...
    distinct_dependency_relations: int = pydantic.Field(alias="distinctDependencyRelations")
...
class NamedCapture(BaseModel):
    name: str
    label: str
    captured_match: Union[BaseMatch, EventMatch, NamedCaptureMatch] = pydantic.Field(alias="capturedMatch")

class BaseMatch(BaseModel):
    start: int = pydantic.Field(description="Inclusive token index which denotes the start of this match's span.")
    end: int = pydantic.Field(description="Exclusive token index which denotes the end of this match's span.")
...
class ScoreDoc(BaseModel):
    sentence_id: int = pydantic.Field(alias="sentenceId", description="The internal ID for this Odinson Document.")
    score: float = pydantic.Field(description="The Lucene score for this Document.")
    document_id: str = pydantic.Field(alias="documentId", description="The parent document's ID as provided at index time (uses org.clulab.processors.Document.id)")
...
class Results(BaseModel):
    ...
    metadata_query: Optional[str] = pydantic.Field(alias="metadataQuery", description="A query to filter Documents by their metadata before applying an Odinson pattern. See https://docs.lum.ai/odinson/metadata for details.", default=None)
    duration: float = pydantic.Field(description="The query's execution time (in seconds)")    
    total_hits: int = pydantic.Field(alias="totalHits", description="The total number of hits (matches) for the query")
    score_docs: List[ScoreDoc] = pydantic.Field(alias="scoreDocs", description="The matches")
